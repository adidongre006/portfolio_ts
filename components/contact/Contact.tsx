// ​"use client";
import { useRef, useState } from "react";
import { motion, useInView } from "framer-motion";
import emailjs from "@emailjs/browser";
/* ─────────────────────────────────────────────
Animation variants (mirrors original logic)
───────────────────────────────────────────── */
const containerVariants = {
initial: { y: 500, opacity: 0 },
animate: {
y: 0,
opacity: 1,
transition: { duration: 1, staggerChildren: 0.1 },
},
};
const childVariants = {
initial: { y: 500, opacity: 0 },
animate: {
y: 0,
opacity: 1,
transition: { duration:1 },
},
};
/* ─────────────────────────────────────────────
Phone SVG path (unchanged from original)
───────────────────────────────────────────── */
const PHONE_PATH =
"M28.189,16.504h-1.666c0-5.437-4.422-9.858-9.856-9.858l-0.001-1.664C23.021,4.979,28.189,10.149,28.189,16.504z"+
"M16.666,7.856L16.665,9.52c3.853,0,6.983,3.133,6.981,6.983l1.666-0.001C25.312,11.735,21.436,7.856,16.666,7.856z " +
"M16.333,0C7.326,0,0,7.326,0,16.334c0,9.006,7.326,16.332,16.333,16.332c0.557,0,1.007-0.45,1.007-1.006 " +
"c0-0.559-0.45-1.01-1.007-1.01c-7.896,0-14.318-6.424-14.318-14.316c0-7.896,6.422-14.319,14.318-14.319 " +
"c7.896,0,14.317,6.424,14.317,14.319c0,3.299-1.756,6.568-4.269,7.954c-0.913,0.502-1.903,0.751-2.959,0.761 " +
"c0.634-0.377,1.183-0.887,1.591-1.529c0.08-0.121,0.186-0.228,0.238-0.359c0.328-0.789,0.357-1.684,0.555-2.518 " +
"c0.243-1.064-4.658-3.143-5.084-1.814c-0.154,0.492-0.39,2.048-0.699,2.458c-0.275,0.366-0.953,0.192-1.377-0.168 "
+
"c-1.117-0.952-2.364-2.351-3.458-3.457l0.002-0.001c-0.028-0.029-0.062-0.061-0.092-0.092 " +
"c-0.031-0.029-0.062-0.062-0.093-0.092v0.002c-1.106-1.096-2.506-2.34-3.457-3.459 " +
"c-0.36-0.424-0.534-1.102-0.168-1.377c0.41-0.311,1.966-0.543,2.458-0.699c1.326-0.424-0.75-5.328-1.816-5.084 " +
"c-0.832,0.195-1.727,0.227-2.516,0.553c-0.134,0.057-0.238,0.16-0.359,0.24c-2.799,1.774-3.16,6.082-0.428,9.292 " +
"c1.041,1.228,2.127,2.416,3.245,3.576l-0.006,0.004c0.031,0.031,0.063,0.06,0.095,0.09 " +"c0.03,0.031,0.059,0.062,0.088,0.095l0.006-0.006c1.16,1.118,2.535,2.765,4.769,4.255 " +
"c4.703,3.141,8.312,2.264,10.438,1.098c3.67-2.021,5.312-6.338,5.312-9.719C32.666,7.326,25.339,0,16.333,0z";
/* ─────────────────────────────────────────────
Contact info items
───────────────────────────────────────────── */
const contactItems = [
{ label: "Mail", value: "adityadongre00006@gmail.com" },
{ label: "Address", value: "Nagpur , Maharashtra" },
{ label: "Phone", value: "+91 97653 24768" },
];
/* ─────────────────────────────────────────────
Component
───────────────────────────────────────────── */
export default function Contact() {
const ref = useRef<HTMLDivElement>(null);
const formRef = useRef<HTMLFormElement>(null);
const [error, setError] = useState(false);
const [success, setSuccess] = useState(false);
const [sending, setSending] = useState(false);
const isInView = useInView(ref, { margin: "-100px" });
const sendEmail = (e: React.FormEvent<HTMLFormElement>) => {
e.preventDefault();
if (!formRef.current) return;
setSending(true);
setError(false);
setSuccess(false);
emailjs
.sendForm(
"service_t69k7lw",
"template_iwju1qd",
formRef.current,
"i6Ptjrb2DSdvIMMB-"
)
.then(
() => {
setSuccess(true);
setSending(false);
formRef.current?.reset();
},
() => {setError(true);
setSending(false);
}
);
};
return (
/* ── Page wrapper ── */
<motion.section
ref={ref}
variants={containerVariants}
initial="initial"
whileInView="animate"
viewport={{ once: false }}
className="
min-h-screen w-full
bg-black-100
flex items-center justify-center
px-4 py-16
"
>
<div
className="
w-full max-w-6xl
flex flex-col md:flex-row
items-center
gap-12 md:gap-16
"
>
{/* ── LEFT: text info ── */}
<motion.div
variants={childVariants}
className="
flex-1 flex flex-col
gap-8 md:gap-10
text-center md:text-left
items-center md:items-start
"
>
<motion.h1
variants={childVariants}
className="
text-white font-extrabold leading-none tracking-tighter
text-5xl sm:text-7xl lg:text-[clamp(4rem,8vw,7rem)]
">
Let&apos;s work
<br />
<span className="text-red-600">together</span>
</motion.h1>
<div className="flex flex-col gap-6 w-full">
{contactItems.map(({ label, value }) => (
<motion.div
key={label}
variants={childVariants}
className="
flex flex-col gap-1
border-l-2 border-red-600
pl-4
"
>
<h2 className="text-orange-400 uppercase text-xs tracking-widest font-semibold">
{label}
</h2>
<span className="text-white/70 text-sm sm:text-base font-light">
{value}
</span>
</motion.div>
))}
</div>
</motion.div>
{/* ── RIGHT: form + phone SVG ── */}
<div className="flex-1 relative w-full max-w-lg">
{/* Animated phone SVG backdrop */}
<motion.div
initial={{ opacity: 1 }}
whileInView={{ opacity: 0 }}
transition={{ delay: 3, duration: 5 }}
className="
absolute inset-0
flex items-center justify-center
pointer-events-none
z-0
"
>
<svg
width="360"
height="360"viewBox="0 0 32.666 32.666"
className="stroke-red-600 opacity-80
sm:w-[420px] sm:h-[420px]"
>
<motion.path
strokeWidth={0.2}
fill="none"
initial={{ pathLength: 0 }}
animate={isInView ? { pathLength: 1 } : { pathLength: 0 }}
transition={{ duration: 10 }}
d={PHONE_PATH}
/>
</svg>
</motion.div>
{/* Contact form */}
<motion.form
ref={formRef}
onSubmit={sendEmail}
initial={{ opacity: 0 }}
whileInView={{ opacity: 1 }}
transition={{ delay: 6, duration: 1 }}
className="
relative z-10
flex flex-col gap-4
w-full
"
>
{/* Name */}
<input
type="text"
name="name"
required
placeholder="Name"
className="
w-full px-5 py-4
bg-transparent
border border-white/20 rounded-md
text-white placeholder-white/40
text-sm sm:text-base
focus:outline-none focus:border-orange-500
transition-colors duration-200
"
/>{/* Email */}
<input
type="email"
name="email"
required
placeholder="Email"
className="
w-full px-5 py-4
bg-transparent
border border-white/20 rounded-md
text-white placeholder-white/40
text-sm sm:text-base
focus:outline-none focus:border-orange-500
transition-colors duration-200
"
/>
{/* Message */}
<textarea
name="message"
rows={8}
placeholder="Message"
className="
w-full px-5 py-4
bg-transparent
border border-white/20 rounded-md
text-white placeholder-white/40
text-sm sm:text-base
focus:outline-none focus:border-orange-500
transition-colors duration-200
resize-none
"
/>
{/* Submit */}
<button
type="submit"
disabled={sending}
className="
w-full py-4
bg-orange-500 hover:bg-orange-400
disabled:opacity-60 disabled:cursor-not-allowed
text-white font-semibold text-sm sm:text-base
rounded-md
transition-colors duration-200cursor-pointer
"
>
{sending ? "Sending…" : "Submit"}
</button>
{/* Feedback messages */}
{error && (
<p className="text-red-400 text-sm text-center">
Something went wrong. Please try again.
</p>
)}
{success && (
<p className="text-green-400 text-sm text-center">
Message sent successfully!
</p>
)}
</motion.form>
</div>
</div>
</motion.section>
);
}